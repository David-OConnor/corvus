...
fn EKF_init(nGyro: f32, nAcc: f32, nMag: f32)
{
  ...
  let dv4 = [ 1.0E-6, 1.0E-6, 1.0E-6, 1.0E-6, 1.0E-8, 1.0E-8,
    1.0E-8 ];

  let dv5 = [ nGyro * nGyro, nGyro * nGyro, nGyro * nGyro, nGyro * nGyro, 1.0E-9,
    1.0E-9, 1.0E-9 ];

  let dv6 = [ nAcc * nAcc, nAcc * nAcc,
  ...
  memset(&P[0], 0, 49 * sizeof(double));
  for i in 0..7 {
    P[i + 7 * i] = dv4[i];
  }

  /*  Process noise matrix */
  memset(&Q[0], 0, 49 * sizeof(double));
  for i in 0..7 {
    Q[i + 7 * i] = dv5[i];
  }

  /*  Measurement noise matrix */
  memset(&R[0], 0, 36 * sizeof(double));
  for i in 0..6 {
    R[i + 6 * i] = dv6[i];
  }
        }
...
unsafe fn updateEKFQuatAtt(gyr_rps: [f32; 3], acc_mps2: [f32; 3], mag_unit: [f32; 3], Va_mps: f32,
                    magDecRad: f32,
                      T: f32, NdivT: f32, roll_deg: &f32, pitch_deg: &f32, yaw_deg: &f32)
{
    let mut mnorm = 0.;
    let mut i0 = 0;
  int iy;
...
fn updateEKFQuatAtt(gyr_rps, acc_mps2, mag_unit, Va_mps, magDecRad, T, NdivT) -> (f32, f32, f32) {
