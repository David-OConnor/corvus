...
use crate::drivers::elrs::EprEvent::EPR_FIRST;
...
fn setRssiChannelData(rcData: &[u16])
{
 ...
 */
fn unpackChannelDataHybridWide(rcData: &mut [u16], payload: &[u8])
{
    unpackAnalogChannelData(rcData, payload);
    let switchByte: u8 = payload[5];
...
fn minLqForChaos() -> u8
{
    ...
    let interval: u8 = receiver.modParams.fhssHopInterval;
    interval * ((interval * numfhss + 99) / (interval * numfhss))
}

fn setRFLinkRate(index: u8)
{
// #if defined(USE_RX_SX1280) && defined(USE_RX_SX127X)
    receiver.modParams = (rxExpressLrsSpiConfig().domain == FreqDomain::ISM2400) ? &airRateConfig[1][index] : &airRateConfig[0][index];
    receiver.rfPerfParams = (rxExpressLrsSpiConfig().domain == FreqDomain::ISM2400) ? &rfPerfConfig[1][index] : &rfPerfConfig[0][index];
// #else
...
// #endif
    ...
    receiver.cycleIntervalMs = (11 * getFHSSNumEntries() * receiver.modParams.fhssHopInterval * receiver.modParams.interval) / (10 * 1000);
...
// #ifdef USE_RX_LINK_QUALITY_INFO
    rxSetRfMode(RfRate::RATE_4HZ as u8 - receiver.modParams.enumRate as u8);
// #endif
}

fn handleFHSS() -> bool
{
   let modresultFHSS: u8 = (receiver.nonceRX + 1) % receiver.modParams.fhssHopInterval;
    ...
    let modresultTLM: u8 = (receiver.nonceRX + 1) % (tlmRatioEnumToValue(receiver.modParams.tlmInterval));
...
fn shouldSendTelemetryResponse() -> bool
{
    let modresult: u8 = (receiver.nonceRX + 1) % tlmRatioEnumToValue(receiver.modParams.tlmInterval);
    if (receiver.connectionState == ConnectionState::DISCONNECTED) || (receiver.modParams.tlmInterval == TlmRatio::TLM_RATIO_NO_TLM) ||
...
fn handleSendTelemetryResponse()
{
    let mut packet = [8_u8];
    ...
    unsafe {
        if nextTelemetryType == ELRS_TELEMETRY_TYPE_LINK || !isTelemetrySenderActive() {
            ...
            packet[2] = if receiver.rssiFiltered > 0 {
            0} else { -receiver.rssiFiltered }; //diversity not supported
...
fn updatePhaseLock()
{
    if receiver.connectionState != ELRS_DISCONNECTED && expressLrsEPRHaveBothEvents() {
        let maxOffset: i32 = receiver.modParams.interval / 4;
...
//hwTimerCallbackTick
fn expressLrsOnTimerTickISR() // this is 180 out of phase with the other callback, occurs mid-packet reception
{
    updatePhaseLock();
    receiver.nonceRX += 1;
...
fn tentativeConnection(timeStampMs: u32)
{
...
fn  gotConnection(timeStampMs: u32)
{
    if receiver.connectionState == ELRS_CONNECTED {
...
fn initializeReceiver()
{
    ...
    receiver.uplinkLQ = 0;
    unsafe {
        receiver.rateIndex = if receiver.inBindingMode {
        bindingRateIndex
 ...
 **/
fn processRFMspPacket(packet: &[u8])
{
...
fn processRFSyncPacket(packet: &[u8], timeStampMs: u32) -> bool
{
    ...
    let modelMatched: bool = packet[6] == (receiver.UID[5] ^ modelXor);
...
fn processRFPacket(payload: &[u8], timeStampUs: u32) -> RxSpiReceived
{
    let mut packet = [0_u8; ELRS_RX_TX_BUFF_SIZE];

    receiver.receiveData(packet, ELRS_RX_TX_BUFF_SIZE);

    let type_: PacketType = packet[0] & 0x03;
 ...
 * and blink LED
 */
fn cycleRfMode(timeStampMs: u32)
{
...
//setup
fn expressLrsSpiInit(rxConfig: &RxSpiConfig, rxRuntimeState: &RxRuntimeState, extiConfig: &RxSpiExtiConfig) -> bool
{
    ...
    ISM2400 => {
        ...
        unasfe { bindingRateIndex = ELRS_BINDING_RATE_24 };
    }
    _ => return false;
    }
...
fn handleConfigUpdate(timeStampMs: u32)
{
...
fn handleLinkStatsUpdate(timeStampMs: u32)
{
    if (timeStampMs - receiver.statsUpdatedAtMs) > ELRS_LINK_STATS_CHECK_MS {
        ...
        if receiver.connectionState == ELRS_CONNECTED {
           ...
           let rssiScaled: u16 = scaleRange(constrain(receiver.rssiFiltered, receiver.rfPerfParams.sensitivity, -50), receiver.rfPerfParams->sensitivity, -50, 0, 1023);
...
fn handleTelemetryUpdate()
{
    if receiver.connectionState != ConnectionState::CONNECTED || (receiver.modParams.lmInterval == TlmRatio::NO_TLM) {
        return;
    }
...
{
    if rcData && payload {
        rxExpressLrsSpiConfig().switchMode == SwitchMode::HYBRID_WIDE { unpackChannelDataHybridWide(rcData, payload) } else {
...
fn expressLrsDataReceived(payload: &[u8]) -> RxSpiReceived
{
    ...
    let irqReason: u8 = receiver.rxISR(unsafe { isrTimeStampUs });
    ...
    if receiver.fhssRequired {
        ...
        if (tlmReq) {
            ...
            uint32_t processingTime = (micros() - unsafe { isrTimeStampUs }) % receiver.modParams.interval;
